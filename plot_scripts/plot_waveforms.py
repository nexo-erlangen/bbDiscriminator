...
def main():
    ...
    for idx in xrange(number):
        ...
        if mode == 'single':
            ...
            if wf.shape[0] == 2: #plot_waveforms_U(np.asarray(wf), idx, particleID, eventInfo['LXeEnergy'], folderOUT)
                plot_waveforms_U(np.asarray(wf), idx, particleID, eventInfo['MCEnergy'], folderOUT)
            elif wf.shape[0] == 4:
    ...
    if mode == 'sum':
        for i in range(len(wf_sum)):
            ...
            plot_waveforms_heat(np.asarray(wf_sum[i]), folderOUT + 'heat-%s-%s-%s-%s-%s-P%s.png'%(source, DataOrMC, position, wires, str(i), EXOPHASE))
    return
